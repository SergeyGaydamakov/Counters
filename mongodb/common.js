...
function build(cfg, user_name, user_password){
	console.log("Start initiating replica set <"+cfg._id+">");
	...
	try {
        ...
        if (!createAdminUserOfCluster( user_name, user_password, cfg._id )) {
            console.log("");
            return false;
        }
    } catch (e) {
...
function CreateCluster( shards ){
    ...
    if (!listShards.shards.length) {
        console.log("Cluster has shards: ", listShards.shards.map(shard => shard._id+"("+shard.host+ " : "+shard.state+")").join(", "));
    } else {
